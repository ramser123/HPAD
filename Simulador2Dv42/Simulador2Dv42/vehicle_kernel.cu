...
// ok
__device__ bool isColissionForVehicle(TColor *dst, int id, int imageW, int imageH, int x, int y, int dx, int dy){
...
//ok
__device__ void getFirstStepForVehicle(TColor *dst, int id, int imageW, int imageH, int x, int y, int &px, int &py){
...
//OK
__device__ void frontSidersForVehicle(int id, int rx, int ry, int &dx, int &dy){
...
//OK
__device__ bool isFrontCollisionForVehicle(TColor *dst, int id, int imageW, int imageH, int px, int py, int x, int y, int dx, int dy, int rightSize, int leftSize){
...
__device__ void getNextStepForVehicle(TColor *dst, int id, int imageW, int imageH, int x, int y, int &px, int &py, int *devTraceX, int *devTraceY, float *dev_sizeX_sizeZ_maxSpeed_maxAcce_maxTimeout){
...
__global__ void VehiclePhase0(
    ...
    if(id < maxVehicle)
	{
		...
		{
			int cellSize=maxVehicle*dev_sizeX_sizeZ_maxSpeed_maxAcce_maxTimeout[2];
...
__global__ void VehiclePhase1(
    ...
    if(id < maxVehicle){
		...
		int delay=isSemaphoreRed(tex2D(texImage, (float)devNextX[id] + 0.5f, (float)devNextY[id] + 0.5f), semaphore, 2);
		if(delay==-1)
			return;
		...
		int x=0;
		int y=0;
		if (!getNextDirection(id , x, y, devLocalX, devLocalY, devLocalStep, devNextX, devNextY, 2))
			return;

		int px=devNextX[id];
		int py=devNextY[id];
		getNextStepForVehicle(dst, id, imageW, imageH, x, y, px, py, devTraceX[id], devTraceY[id], dev_sizeX_sizeZ_maxSpeed_maxAcce_maxTimeout);
		...
		if (px != devNextX[id] || py != devNextY[id]){ //nueva posicion
			...
			dst[imageW * devNextY[id] + devNextX[id]] = make_color(1.f, 1.f, 1.f, ((float)id)/255.0f);
			...
			if ( hyp < 2.f ){
				...
				if (devLocalStep[id]!=devMaxLocalStep[id]){
					...
					// comentar las dos lineas de abajo y observar
					float4 nextFresult = tex2D(texImage, (float)devNextX[id] + 0.5f, (float)devNextY[id] + 0.5f);
					dst[imageW * devNextY[id] + devNextX[id]] = make_color(nextFresult.x, nextFresult.y, nextFresult.z, 0.0f);
				}
			}
...
__global__ void VehiclePhase2( 
    ...
    const int id = blockDim.x * blockIdx.x + threadIdx.x;
    if((id < maxVehicle) && (devLocalStep[id] >= 0)){
		...
		if (devConflicted[imageW * devNextY[id] + devNextX[id]] == id){ //talves tenga conflicto, pero tiene prioridad sobre los demas
			...
			drawAllTrace(dst, color, imageW, imageH, devNextX[id], devNextY[id], devNextX[id]-devPreviousX[id], devNextY[id]-devPreviousY[id],
...
__global__ void VehicleCollision(
    TColor *dst,
    int imageW,
    int imageH,
	int maxVehicle,
...
__global__ void VehiclePhase2OLD( 
    TColor *dst,
    int imageW,
    int imageH,
	int maxVehicle,
...
__global__ void VehiclePhase3OLD( 
    TColor *dst,
    int imageW,
    int imageH,
	int maxVehicle,
...
extern "C" void run_Vehicle(
	...
	dim3 dimGrid(maxVehicle, 1); //define en total cuantas veces se ejecuta. (multiplicacion de ambos)
	...
	if(parallelDetection){
		...
		VehiclePhase0<<<dimGrid, dimBlock>>>(maxVehicle, devTimeOut, dev_sizeX_sizeZ_maxSpeed_maxAcce_maxTimeout);
		
		//Fase 1: realizo movimientos así hayan colisiones.
		VehiclePhase1<<<dimGrid, dimBlock>>>(d_dst, imageW, imageH, maxVehicle, semaphore, 
		...
		VehiclePhase2<<<dimGrid, dimBlock>>>(d_dst, imageW, imageH, maxVehicle, semaphore, 
		...
		VehiclePhase3<<<dimGrid, dimBlock>>>(d_dst, imageW, imageH, maxVehicle, semaphore,
												  devLocalStep,
												  devCurrentX, devCurrentY, devPreviousX, devPreviousY,
												  devTraceX, devTraceY, devTraceRotX, devTraceRotY,
												  devConflicted,devTimeOut,dev_sizeX_sizeZ_maxSpeed_maxAcce_maxTimeout);
		...
		VehicleCollision<<<1,1>>>(d_dst, imageW, imageH, maxVehicle, semaphore, 
		...
		//Fase 2: detecto y corrigo movimientos de peatones en conflicto con nuevos movimientos, aun asi pueden haber conflictos en los nuevos movimientos
		VehiclePhase2OLD<<<dimGrid, dimBlock>>>(d_dst, imageW, imageH, maxVehicle, semaphore, 
													 devLocalX, devLocalY, devLocalStep,
													 devCurrentX, devCurrentY, devPreviousX, devPreviousY, devConflicted,
													 devTraceX, devTraceY, devTraceRotX, devTraceRotY,
		...
		VehiclePhase3OLD<<<1,1>>>(d_dst, imageW, imageH, maxVehicle, semaphore, 
									   devLocalStep,
									   devCurrentX, devCurrentY, devPreviousX, devPreviousY, devConflicted,
									   devTraceX, devTraceY, devTraceRotX, devTraceRotY,
									   devClass, devTimeOut, dev_sizeX_sizeZ_maxSpeed_maxAcce_maxTimeout);
